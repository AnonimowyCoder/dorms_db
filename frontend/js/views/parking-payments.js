...
export async function render(container) {
    container.innerHTML = `
        ...
        <div id="payment-form-container" class="hidden" style="margin-top: 30px; border: 1px solid #ccc; padding: 20px; max-width: 400px; background-color: #fff;">
            <h3 id="form-title">Add New Parking Payment</h3>
...
async function loadDependencies() {
    try {
        const [resReservations, resParkingLots, resResidents] = await Promise.all([
            fetchWithAuth('/parking-reservations'),
        ...
        if (reservationsData.length > 0) {
            resSelect.innerHTML = '<option value="">Select reservation...</option>' +
                reservationsData.map(r => `<option value="${r.id}">Res ID: ${r.id} (Parking ID: ${r.id_parking_lot})</option>`).join('');
        } else {
...
async function loadPayments() {
    ...
    try {
        const response = await fetchWithAuth('/parking-payments');
        if (!response.ok) throw new Error('Failed to fetch parking payments');
...
function renderTable() {
    ...
    if (paymentsData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7">No parking payments found.</td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = paymentsData.map(payment => {
        ...
        return `
            <tr>
                <td>${payment.id}</td>
                <td>
                    ID: ${payment.id_parking_reservation}
                    <button class="info-btn" data-resid="${payment.id_parking_reservation}" style="margin-left: 8px; cursor: pointer;">Details</button>
                </td>
    ...
    if (!res) {
        ...
        contentDiv.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                ...
                <div><strong>Parking Lot:</strong><br>${lotDisplay}</div>
...
function showAddForm() {
    document.getElementById('form-title').innerText = 'Add New Parking Payment';
...
function showEditForm(id) {
    ...
    document.getElementById('form-title').innerText = 'Edit Parking Payment';
    document.getElementById('payment-id').value = payment.id;

    const resSelect = document.getElementById('payment-reservation-id');
    resSelect.value = payment.id_parking_reservation;
...
async function handleFormSubmit(e) {
    ...
    if (!isEditing) {
        payload.id_parking_reservation = parseInt(document.getElementById('payment-reservation-id').value, 10);
    }
    ...
    try {
        let response;
        if (isEditing) {
            response = await fetchWithAuth(`/parking-payments/${id}`, {
                method: 'PATCH',
                body: JSON.stringify(payload)
            });
        } else {
            response = await fetchWithAuth('/parking-payments', {
                method: 'POST',
                body: JSON.stringify(payload)
            });
        }
        ...
        if (!response.ok) {
            ...
            let errorMsg = 'Failed to save parking payment.';
        ...
        messageDiv.innerHTML = `<span style="color: green;">Parking payment ${isEditing ? 'updated' : 'created'} successfully.</span>`;
        hideForm();
        await loadPayments();
    } catch (error) {
...
async function deletePayment(id) {
    if (!confirm('Are you sure you want to delete this parking payment record?')) return;

    const messageDiv = document.getElementById('payment-message');

    try {
        const response = await fetchWithAuth(`/parking-payments/${id}`, {
            method: 'DELETE'
        });

        if (!response.ok) throw new Error('Failed to delete parking payment');

        messageDiv.innerHTML = `<span style="color: green;">Parking payment deleted successfully.</span>`;
        await loadPayments();
    } catch (error) {
        messageDiv.innerHTML = `<span style="color: red;">Error deleting parking payment: ${error.message}</span>`;
    }
}
